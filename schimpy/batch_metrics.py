...
from schimpy.metricsplot import (
    plot_metrics,
    plot_comparison,
    get_common_window,
    safe_window,
    check_if_all_tss_are_bad,
    fill_gaps,
)
...
class BatchMetrics(object):
    ...
    def read_simulation_outputs(
        self, variable, outputs_dir, time_basis, stations_input=None
    ):
            ...
            for i, working_dir in enumerate(outputs_dir):
                if stations_input is None:
                    if os.path.basename(working_dir) == "outputs":
                        station_in = os.path.join(
                            os.path.dirname(working_dir), "fluxflag.prop")
                    else:
    ...
    def retrieve_ts_obs(
        self, station_id, subloc, variable, window, db_stations, db_obs
    ):
        ...
        if fpath_obs:
            if os.path.exists(fpath_obs) or len(glob.glob(fpath_obs)) > 0:
                ...
                if ts_obs is None or len(ts_obs) == 0:
                    ...
                    self.logger.warning(
                        "File {} does not contain useful data for the requested time window".format(
                            os.path.abspath(fpath_obs)
                        )
                    )
                    ts_obs = None
                return ts_obs
                ...
                self.logger.warning(
                    "Data file not found on file system: {}".format(
                        os.path.abspath(fpath_obs)
                    )
                )
                return None
    ...
    def create_title(self, db_stations, station_id, source, variable, subloc):
        ...
        if variable in ("salt", "temp"):
            if subloc != "default":
                title += " ({})".format(subloc)
        title += "\n"
        title += "Source: {}, ID: {}\n".format(source.upper(), station_id.upper())
        return title
    ...
    def plot(self):
        ...
        if stations_input is None:
            stations_input = (
                params.get("flow_station_input")
                if variable == "flow"
                else params.get("station_input")
            )
        else:
            raise ValueError(
        ...
        window_to_read = (window_common[0] - padding, window_common[1] + padding)
        ...
        sim_outputs = self.read_simulation_outputs(
            variable, outputs_dir, time_basis, stations_input
        )
        ...
        for stn in sim_outputs[0].columns:
            ...
            if not station_id.lower() in db_stations.index:
                self.logger.warning(
                    "Station id {} not found in station listings ({})".format(
                        station_id, params["stations_csv"]
                    )
                )
                continue
            ...
            ts_obs = self.retrieve_ts_obs(
                station_id, subloc, variable, window_to_read, db_stations, db_obs
            )

            if ts_obs is None or ts_obs.isnull().all():
                self.logger.warning(
                    f"No observation data: {station_id}, {subloc}, {variable} ({params['obs_links_csv']})."
                )
                ...
                if remove_outliers is True:
                    ...
                    if variable == "salt" and obs_unit in ["microS/cm", "uS/cm"]:
                        ...
                        drange = None
                    ts_obs = med_outliers(ts_obs, level=4, range=drange, copy=False)
            ...
            if variable == "flow":
                tss_sim = [
                    None if simout[station_id].isnull().all() else simout[station_id]
                ...
                try:
                    ...
                    self.logger.warning(
                        "Problem reading time series for station {},{}".format(
                            station_id, subloc
                        )
                    )
                    raise
            ...
            if adjust_datum and ts_obs is not None:
                ts_obs, adj = self.adjust_obs_datum(
                    ts_obs, tss_sim[0], station_id, variable, subloc, db_obs
                )
                adj_obs += adj
            ...
            if "palette" in params.keys():
                if params["palette"] == "default":
                    ...
                    style_palette = (
                        ...
                        * cycler(
                            color=["#0F0F0F"]
                            + matplotlib.style.library[params["palette"]][
                                "axes.prop_cycle"
                            ].by_key()["color"]
                        )
                    ...
                    if "custom_palette" in params.keys():
                        if "linecolor" in params["custom_palette"]:
                            ...
                            if linecolor == "default":
                                linecolor = default_style_cycler.by_key()["color"][
                                    : len(labels)
                                ]
                            elif linecolor in plt.style.available:
                                linecolor = ["#0F0F0F"] + matplotlib.style.library[
                                    linecolor
                                ]["axes.prop_cycle"].by_key()["color"][: len(labels)]
                            elif len(linecolor) < len(labels):
                                raise ValueError(
                                    "Specify correct number of linecolors!"
                                )
                            ...
                            print(
                                "'linecolor' not specified in 'custom_palette'. Default used"
                            )
                        ...
                        if "linewidth" in params["custom_palette"]:
                            ...
                            print(
                                "'linewidth' not specified in 'custom_palette'. Default used"
                            )

                        style_palette = cycler(
            ...
            if "write_csv" in params.keys():
                if params["write_csv"]:
                    ...
                    title_first_line = figtitle.split("\n")[
                        0
                    ]  # can't seem to handle multiple lines in yaml even with block output
            ...
            fname_output = self.set_fname_out(alias, variable, station_id, vert_pos)
