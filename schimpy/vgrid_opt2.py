...
def targeth2inv(zbar, eta):
    href = 1.0 + (eta - zbar) / 60.0
    href2inv = 1.0 / (href * href)
...
def meshobj(
    ...
    href2inv2 = 1.0 / (href * href)
    href2inv2[np.isinf(href2inv2)] = 0.0

    # vertical spacing
    zdiff2norm = href2inv2 * zdiff**2.0
    zdiff2norm[nodemasked, :] = 0.0
    obj += np.sum(zdiff2norm)

    # minimum spacing
    lht = -zdiff - (foldfrac * href)
    # lht = np.nan_to_num(lht)
    bad = np.minimum(lht, 0.0)
    bad[nodemasked, :] = 0.0
...
def meshessp(
    ...
    folded_below = (
        ndx >= 0
    )  # just a starting point, not the folded computation (see below)
    folded_above = ndx >= 0
    ...
    if len(ndxf) > 0 and ndxf[-1] == (len(prod) - 1):
        ndxf = ndxf[:-1]
...
def hess_base(
    ...
    edgelen2inv = np.tile(1.0 / sidelen2[~sidemasked], (maxlevel, 1)).transpose()
    ...
    diag1[:, :-1] += href2inv2
    diag1[:, 1:] += np.where(ndx[:, 1:] >= 0, href2inv2, 0.0)
    diag2[:, 0:-1] -= np.where((ndx[:, 1:] >= 0) & (ndx[:, :-1] >= 0), href2inv2, 0.0)

    href_hess = diags(
        (diag1[ndx >= 0].ravel(), diag2[ndx >= 0].ravel(), diag2[ndx >= 0].ravel()),
        [0, 1, -1],
    ).tocsr()
...
def test_vgrid_spacing(zcor, zcorig, nodemasked, foldfrac):
    ...
    wherebad = np.where((bad < 0.0) & (href > 1e-14))
    print("Any bad?")
    print(wherebad)
...
def mesh_opt(
    ...
    print(
        "Optimization succeeded: %s with objective value %s"
        % (result.success, result.fun)
    )
    print(
        "Number iterations: %s Func: %s  Grad: %s"
...
def smooth_heights(zcor, x, wgt=0.2):
    ...
    dh[np.isnan(dh)] = 0.0  # fill vlues
    ddh = np.diff(dh, axis=0)
    incr = (
        ddh[1:, :] * weight_hi[:, np.newaxis] - ddh[:-1, :] * weight_lo[:, np.newaxis]
    )
    dh2 = dh[1:-1, :] + wgt * incr
    dh2[dh[1:-1, :] == 0.0] = 0.0
    zcornew = zcor.copy()
...
def laplace2(mesh, nodemasked, sidemasked):
    ...
    for i in np.arange(nnode):
        ...
        if len(e) == 0:
            continue
        other = [i] + [
            (sides[nn][0] if sides[nn][0] != i else sides[nn][1]) for nn in e
        ]
    ...
    jj = np.concatenate((jj))
    data = np.concatenate((data))
    weightcoo = coo_matrix(
        (data.ravel(), (ii.ravel(), jj.ravel())), shape=(nnode, nnode)
    )
...
def gradient_matrix(mesh, sidedist2, sidemasked):
    ...
    first = edgesgood[:, 0]
    second = edgesgood[:, 1]
    s2 = 1.0 / np.sqrt(sidedist2[~sidemasked])
...
def example():
    ...
    dx2fac = 10.0  # 1000.
    curvewgt = 100.0  # 0.01 # 0.05 #0.05 #0.05 #0.03 #800000
    foldwgt = 22.0  # 1.e3
    foldfrac = 0.35
    maxiter = 200
    # gr3name = "gg_cut.gr3"
    ...
    nx = x.shape[0]
    sidelen2 = np.sum((x[edges[:, 1], :] - x[edges[:, 0], :]) ** 2.0, axis=1)
