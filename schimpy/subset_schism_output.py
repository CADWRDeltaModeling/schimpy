    ...
    for ii in range(nFacesROI):
        for jj in nMax_f_nodes:
            if not (isnan(temp_f_nodes_local_vals[ii][jj])):
                face_to_nodes_local[ii][jj] = node_map_list[
                    temp_f_nodes_local_vals[ii][jj]
                ]
    ...
    if vgrid_exist:
        ...
        if ivcor == 1:
            ...
            with open(subset_vgrid_file, "w") as subset_vgrid:
                ...
                np.savetxt(
                    subset_vgrid, subset_bottom.reshape(1, -1).astype(int), fmt="%i"
                )
