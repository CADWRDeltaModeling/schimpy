...
def convert_2dm(file, outfile=None, elev2depth=False, logger=None):
    ...
    nodestrlines = [
        line.strip() for line in all_lines[last_possible_node:] if line.startswith("NS")
    ]
    ...
    bc_regex = re.compile(
        r"""BC\s+\d+\s+\"(land|open|island)\"\s+(\d+).*\nBC_DEF\s+\d+\s+\d+\s+\"(.*)\"\s+\d+\s+\"(.*)\".*"""
    )
    ...
    for line in boundlines:
        if line.startswith("BC_VAL"):
            ...
            bc = Boundary(
                name,
                boundary_def[1],
                np.array(boundnodestrings[entity_id - 1], dtype=np.int32),
            )
    ...
    for i, bound in enumerate(openbound):
        fout.write(
            "%s !Number of nodes for open boundary (%s) %s\n"
    ...
    fout.write(
        "%s !Number of land boundary nodes (including islands)\n" % nlandboundnode
    )
    for i, bound in enumerate(landbound):
        fout.write(
            "%s !Number of nodes for land boundary %s ('0' means exterior land boundary)\n"
    ...
    for i, bound in enumerate(islebound):
        fout.write(
            "%s !Number of nodes for island boundary %s ('1' means island)\n"
            % (len(bound.nodestring), i + 1)
        )
        np.savetxt(fout, bound.nodestring, fmt="%i")
    fout.close()
