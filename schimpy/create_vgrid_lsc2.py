...
def main():
        ...
        with open(transect, "r") as f:
            ...
            for fname in transectfiles:
                if not os.path.exists(fname):
                    raise ValueError(
                        "Requested output transect file does not exist: {}".format(
                            fname
                        )
                    )

    vgrid_gen(
        hgrid, vgrid_out, vgrid_version, eta, minmax_region, archive_nlayer, nlayer_gr3
    )

    transect_mallard = ["mallard_1.csv", "mallard_2.csv"]
    transect_gg = ["transect_gg1.csv", "transect_gg2.csv"]
    transect_liberty = [
        "toe_drain_north_liberty_1.csv",
        "toe_drain_north_liberty_2.csv",
        "toe_drain_north_liberty_3.csv",
        "toe_drain_north_liberty_4.csv",
    ]
    transect_pinole = [
        "pinole_shoal_1.csv",
        "pinole_shoal_2.csv",
    ...
    transect_franks = [
        "frank_tract_sjr_1.csv",
        "frank_tract_sjr_2.csv",
        "frank_tract_sjr_3.csv",
        "frank_tract_sjr_4.csv",
...
def vgrid_gen(
    ...
    dummyout = meshfun.depth(dummyk, dummydepth, 0.0)
    ...
    places_on = np.array(
        [[626573.490000, 4260349.590000], [626635.000000, 4260391.7]], dtype="d"
    )
    dists_on = np.min(
        scipy.spatial.distance.cdist(mesh.nodes[:, 0:2], places_on), axis=1
    )
    ...
    dztarget = np.full_like(h0, 100.0, dtype="d")
    ...
    if archive_nlayer == "out":
        ...
        xdummy = 0.0
        nlayer_default = default_num_layers(
            xdummy, eta, h0, minlayer, maxlayer, dztarget, meshfun
        )
