...
class SchismPolygon(Polygon):
    ...
    def __eq__(self, other):
        if not isinstance(other, self.__class__):
            return False
        return self.prop == other.prop and super(self.__class__, self).__eq__(other)
    ...
    def read(self, data):
        ...
        if data is not None:
            for item in data:
                ...
                for k in item:
                    if k == "vertices":
                        vertices = [[float(e) for e in row] for row in item["vertices"]]
                    else:
                        prop[k] = item[k]
                polygons.append(SchismPolygon(orient(Polygon(vertices)), prop=prop))
        return polygons
        ...
        if os.path.exists(fpath):
            ...
            for feature in layer:
                ...
                if name_geom in ("POLYGON", "MULTIPOLYGON"):
                    ...
                    for g in geoms:
                        shell = [point for point in g.GetGeometryRef(0).GetPoints()]
                        prop = {}
                        for i, field_name in enumerate(field_names):
                            prop[field_name] = feature.GetFieldAsString(i)
                        polygons.append(
                            SchismPolygon(orient(Polygon(shell=shell)), prop=prop)
                        )
            return polygons
    ...
    def write(self, fpath, polygons, spatial_reference=None, driver_name=None):
        ...
        if isinstance(spatial_reference, str):
            ...
            try:
                ...
                raise ValueError(
                    "spatial reference could not be created from string: {}".format(
                        spatial_reference
                    )
                )
        ...
        layer = datasource.CreateLayer("layer", spatial_reference_obj, wkbPolygon)
