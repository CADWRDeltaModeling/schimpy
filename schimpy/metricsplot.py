...
def fill_gaps(ts, max_gap_to_fill=None):
    ...
    try:
        ...
        raise ValueError(
            "could not divide max_gap_to_fill by freq: {}".format(ts.index.freq)
        )
    if limit == 0:
...
def plot_metrics_to_figure(
    ...
    axes = dict(
        list(zip(list(grids.keys()), list(map(fig.add_subplot, list(grids.values())))))
    )
    ...
    for i in range(len(tss) - 1):
        if tss[0] is not None:
            ...
            if tss[i + 1] is None or allbad:
                ...
                continue
            try:
                lag = calculate_lag(tss[0], tss[i + 1], max_shift, period)
