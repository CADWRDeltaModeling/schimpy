...
def read_flux(flux="flux.th", flux_head=None, time_basis=None, elapsed_unit="s"):
    ...
    if flux_head is not None:
        ...
        if len(headers) != len(flux_df.columns):
            raise ValueError(
                f"{os.path.basename(flux_file)} has {len(flux_df.columns)} columns and the header calls for {len(headers)}!"
            )
...
def combine_flux(flux, comb_dict, time_basis=None, elapsed_unit="s", flux_head=None):
    """Combine flux data,
    ...
    if not isinstance(flux, (pd.DataFrame, pd.Series)):
        flux_df = read_flux(
            flux=flux, flux_head=flux_head, time_basis=time_basis, elapsed_unit="s"
        )
    else:
        flux_df = flux
