...
def profile_plot(
    ...
    print(
        "min x station %s max x station %s max_depth %s"
        % (min_station, max_station, max_depth)
    )
    ...
    if add_cruise_loc and add_labels:
        ...
        leg = ax.legend(
            ("USGS cast", "Dayflow X2"), "center left", numpoints=1, frameon=False
        )
