...
def partition_check(mesh, poly_fn, regions, centering='node', crs=None,
    ...
    * if any nodes/elems were assigned to multiple polygons.
    ...
    for i, poly in enumerate(poly_gpd['geometry']):
        try:
            id_n = np.argwhere(region_names==poly_gpd.region[i])[0,0] #re-order based on input yaml file
        except IndexError:
            raise Exception(
                "polygon region %s defined in the shapefile but not used by input yaml file."%poly_gpd.region[i])
    ...
    ID_df = ID_df.reindex(sorted(ID_df.columns), axis=1) # sort according to column names
    ...
    #     ID_df[other_id].loc[orphaned_cells] = True
        ...
        if allow_incomplete:
            NotImplementedError("categorizing cells based on nearest distance has not been implemented")
        else:
    ...
    if len(multi_labeled_cells) >= 1:
        if allow_overlap:
            ...
            print('''Multiple cells or nodes belong to more than one polygons %s: 
                  categorized as the last True poly''')
            logging.info("The following cells or nodes belong to more than one \
                         region: %s"%string)
    else:    
