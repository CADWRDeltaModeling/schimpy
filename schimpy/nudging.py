...
class Nudging(object):
    ...
    def concatenate_nudge(self, weights_var, values_var, imap_var,
        ...
        for i, v in enumerate(var_newlist):
            ...
            for im in range(len(imap_merged)):
                ...
                if len(idx_r) > 1:  # overlapping regions
                    ...
                    for r in idx_r:
                        ...
                        data_values = values_var[i][r][:, int(
                            idx_list[im, r]), :]
                    ...
                    if np.shape(values_temp)[0] > len(self.time):
                        values_merged[:, im,
                                      :] = values_temp[:len(self.time), :]
                    else:
                        values_merged[:, im, :] = values_temp
                    weights_merged[imap_merged[im]] = sum(
                        weights_sum2)/sum(weights_sum)
                else:
                    r = idx_r[0]
            ...
            itime_dim = rootgrp_T.createDimension(
                "time", len(self.time_seconds))
    ...
    def create_region_nudging(self, region_info):
        if region_info['type'] == '3dfield':
            weights, values_list, imap, time = self.gen_nudge_3dfield(
                region_info)
    ...
    def gen_nudge_3dfield(self, region_info):
        ...
        nt_out = int(pd.Timedelta(self.nudge_step) /
                     pd.Timedelta(dtout))  # output stride
        ...
        for d in range(nndays):
            ...
            for t in time:
                ...
                if wetij[0].size == 0:
                    write_to_log(
                        "no 3dfield  data available for: %s\n" % str(ctime))
                            ...
                            if (kbp[ix[il], iy[il]] == -1):  # 3dfield dry cell
                                ...
                                for kk in range(ilen-1):
                                    if (self.z[ii, k] >= zm[kk] and
                                        ...
                                        lev[il, k] = kk
                                        vrat[il, k] = (
                                            zm[kk]-self.z[ii, k])/(zm[kk]-zm[kk+1])
                                        break
                            if lev[il, k] == -99:
                ...
                wild2[:, :, 0, 0] = temp[ix, iy, lev] * \
                    (1-vrat)+temp[ix, iy, lev+1]*vrat
                wild2[:, :, 0, 1] = salt[ix, iy, lev] * \
                    (1-vrat)+salt[ix, iy, lev+1]*vrat
                wild2[:, :, 1, 0] = temp[ix+1, iy, lev] * \
                    (1-vrat)+temp[ix+1, iy, lev+1]*vrat
                wild2[:, :, 1, 1] = salt[ix+1, iy, lev] * \
                    (1-vrat)+salt[ix+1, iy, lev+1]*vrat
                wild2[:, :, 2, 0] = temp[ix+1, iy+1, lev] * \
                    (1-vrat)+temp[ix+1, iy+1, lev+1]*vrat
                wild2[:, :, 2, 1] = salt[ix+1, iy+1, lev] * \
                    (1-vrat)+salt[ix+1, iy+1, lev+1]*vrat
                wild2[:, :, 3, 0] = temp[ix, iy+1, lev] * \
                    (1-vrat)+temp[ix, iy+1, lev+1]*vrat
                wild2[:, :, 3, 1] = salt[ix, iy+1, lev] * \
                    (1-vrat)+salt[ix, iy+1, lev+1]*vrat
                ...
                idST = np.where((kbp[ix, iy] != -1) &
                                (self.z[i_in, :].T > -10))
                tempout[idST[0], i_in[idST[1]]
                        ] = tempout[idST[0], i_in[idST[1]]]-1
                ...
                nudge_step = int(pd.Timedelta(
                    self.nudge_step).total_seconds()/3600)
                if len(output_day) > 1:
    ...
    def read_data(self, data):
        if data.endswith('csv'):
            obs = pd.read_csv(data, index_col='datetime',
        ...
        if isinstance(attribute, str):
            ...
            if isinstance(x0, np.ndarray):
                ...
                for i, (xc, yc) in enumerate(zip(x0, y0)):
                    weights_c = self.gaussian_weights(
                        [xc, yc], [x0, y0], attribute)
                    # perhaps other more sophisticated normalization can be used here instead
                    if sum(weights_c) > 1:
                        norm_factor[i] = weights_c[i] / \
                            sum(weights_c)  # if 1 exceeds
            else:
                weights = np.zeros_like(self.node_x)
                norm_factor = 1
            if vertices != 'None':
