...
class hotstart(object):
    ...
    def read_yaml(self, use_logging=True):
        ...
        if "crs" in hotstart_info.keys():
            ...
            if self.crs is None:
                self.crs = "EPSG:26910"
                print(
                    "crs not specified, and assigned to the default crs of EPSG:26910"
                )
        ...
        if self.modules is None:
            if "modules" in hotstart_info.keys():
                ...
                print(
                    "No module specified. Baroclinic run assumed. 'TEM', and 'SAL' turned on as default"
                )
                modules = ["TEM", "SAL"]
            if modules is None:
            ...
            if self.modules is not None:
                if "SED" in self.modules:
                    if (
                        "sediment_input_file" not in hotstart_info.keys()
                        or not os.path.isfile(hotstart_info["sediment_input_file"])
                    ):
        ...
        if "output_fn" in hotstart_info.keys():
            ...
            print(
                "No output file name specified. hotstart.restart_timestr.nsteps.nc will be used."
            )
            self.output_fn = None
        ...
        if (
            self.modules
        ):  # if modules is None for barotropic run, this step is not required.
            self.ntracers, self.ntrs, self.irange_tr, self.tr_mname = describe_tracers(
                self.param_nml, modules=self.modules
            )
        else:
            self.ntracers = 0
            self.tr_mname = []
    ...
    def create_hotstart(self):
        ...
        for v in self.variables:
            ...
            if (
                ("hotstart_nc" in initializer)
                and (not self.hotstart_ini)
                and ("source_hgrid" in initializer["hotstart_nc"])
            ):
                self.hotstart_ini["hotstart_nc_hfn"] = initializer["hotstart_nc"][
                    "source_hgrid"
                ]
                self.hotstart_ini["hotstart_nc_vfn"] = initializer["hotstart_nc"][
                    "source_vgrid"
                ]
                ...
                if "hotstart_nc" in patch_init:
                    ...
                    sub_init = initializer["patch_init"]["regions"][idx_h][
                        "initializer"
                    ]["hotstart_nc"]
        ...
        if (
            "COSINE" in self.modules
        ):  # COS_mS2=COS_5, COS_mDN=COS_8, COS_mZ1 = COS_6, COS_mZ2 = COS_7
            self.nc_dataset = self.nc_dataset.assign(COS_mS2=self.nc_dataset.COS_5_el)
            self.nc_dataset = self.nc_dataset.assign(COS_mDN=self.nc_dataset.COS_8_el)
            self.nc_dataset = self.nc_dataset.assign(COS_mZ1=self.nc_dataset.COS_6_el)
            self.nc_dataset = self.nc_dataset.assign(COS_mZ2=self.nc_dataset.COS_7_el)
            ...
            self.nc_dataset["COS_sS2"] = (
                ("elem", "nVert"),
                np.zeros_like(self.nc_dataset.COS_1_el.values),
            )
        ...
        # compute depth again for the updated elevation.
                    ...
                    ds_ar = xr.Dataset(
                        {
                            name: (
                                ["node", "nVert"],
                                np.zeros(
                                    (self.mesh.n_nodes(), self.mesh.n_vert_levels)
                                ),
                            )
                        }
                    )
    ...
    def map_to_schism(self):
        ...
        for key, var_values in zip(
            schism_hotstart_var.keys(), schism_hotstart_var.values()
        ):
...
class VariableField(object):
    ...
    def var_centering(self):
        ...
        if variable_type in vc_mapping.keys():
            ...
            raise ValueError(
                "variable %s does not have a valid input centering option"
                % self.variable_name
            )
    ...
    def get_grid(self):
        ...
        if self.centering not in options:
            raise NotImplementedError(
                """centering option %s is not implemented
        ...
        centering_options = {
            ...
            "prism": {
                "node": (
                    self.n_nodes,
                    self.node,
                ),  # on prism center: mainly for tracers; first average from nodes to centers and then then average to prism center
                "nVert": (self.n_vert_levels, self.depths),
            },
            "node2D": {
                "node": (self.n_nodes, self.node),
        ...
        if self.centering == "edge":
            centering_options["edge"] = {
                "side": (self.n_edges, self.edge),
                "nVert": (self.n_vert_levels, self.edge_depths),
            }
        elif self.centering == "elem":
            centering_options["elem"] = {
                "elem": (
                    self.n_elems,
                    self.elem,
                ),  # on element but at whole level: mainly for w
                "nVert": (self.n_vert_levels, self.elem_depths),
            }
    ...
    def define_new_grid(self, mesh):
        ...
        if self.centering == "edge":
            edge_depths = (
                mesh_depth[mesh.edges[:, 0]] + mesh_depth[mesh.edges[:, 1]]
            ) / 2
            centering_options["edge"] = {
                "side": (mesh.n_edges, mesh_edge),
    ...
    def initializer_options(self):
        ...
        if initializer not in options:
            raise NotImplementedError(
                """initializer %s is not implimented
    ...
    def simple_trend(self, ini_meta=None, inpoly=None):
        ...
        if self.variable_name in ["SED3D_bedfrac"]:
            value = str(
                value
            )  # if a single float value is given, it needs to be converted to a string

            # trigger an error if number of values is not equal to nsed
            if len(value.split(",")) != self.nsed:
                raise ValueError(
                    "Number of values in %s does not match with nsed (%d)"
                    % (value, self.nsed)
                )
    ...
    def text_init(self, ini_meta=None, inpoly=None):
        ...
        if text_fn.endswith(".ic") or text_fn.endswith(".gr3"):
            if self.centering == "node2D":
                reader = (
                    SchismMeshGr3Reader()
                )  # the ic file has the same format as the .gr3 file
                ...
                if icmesh.n_nodes() == self.n_nodes:
                    ...
                    raise Exception(
                        "The node of %s is incompatible with the grid node in hgrid.gr3"
                        % text_fn
                    )
                ...
                raise ValueError(
                    "%s gives value on node, but value on %s is required"
            ...
            if self.centering in ["elem", "prism"]:
                ...
                if len(vmap) != self.n_elems:
                    raise ValueError(
                        "The element of {} is incompatible with grid element in hgrid.gr3".format(
                            text_fn
                        )
                    )
                ...
                raise ValueError(
                    "%s gives value on element, but value on %s is required"
                    % (text_fn, self.centering)
                )
            ...
            if nh != self.n_hgrid:
                if self.n_hgrid == self.mesh.n_nodes:
                    ...
                    raise ValueError(
                        "The horizontal dim of %f does not match with the mesh grid"
                        % text_fn
                    )
        else:
            raise TypeError("Only .ic, .gr3, .prop, or .nc are allowed as text input")
    ...
    def interp_from_mesh(
        ...
        if (
            dist_th is not None
        ):  # same points, apply nearest; different points, apply the method defined
            ...
            if isinstance(vin, xr.core.dataset.Dataset):
                vout = np.zeros((len(vin), np.shape(vgrid2)[0], np.shape(vgrid2)[1]))
                if method == "nearest":
                ...
                if method == "nearest":
                    ...
                    if np.ndim(vin) < np.ndim(vout):
                        vout[same_points, :] = vin[indices[same_points]].values[
                            :, np.newaxis
                        ]
                    else:
                        vout[same_points, :] = vin[indices[same_points], :]
            ...
            if self.centering == "node2D" or (
                self.mesh.n_vert_levels == mesh1.n_vert_levels
                and (
                    "vinterp" not in self.ini_meta or self.ini_meta["vinterp"] is False
                )
            ):
                if isinstance(vin, xr.core.dataset.Dataset):
                    vout = np.zeros(((len(vin.keys())), len(indices), len(vin.nVert)))
                    for j, v in enumerate(list(vin.keys())):
                        vout[j, :] = vin[v][indices]
                else:
                    vout = vin[indices]
            else:
                print("interpolating vertical grid from %s" % (vgrid_fn))
                if isinstance(vin, xr.core.dataset.Dataset):
                    vout = np.zeros(
                        (len(vin), np.shape(vgrid2)[0], np.shape(vgrid2)[1])
                    )
                else:
                    vout = np.zeros_like(vgrid2)
...
def describe_tracers(param_nml, modules=["HYDRO"]):
    ...
    if "FABM" in modules:
        raise NotImplementedError(
            "Initializing FABM from hotstart.nc is currently not implemented; \
...
def hotstart_to_outputnc(
    ...
    grid_newname = {
        "n_hgrid_face": "nSCHISM_hgrid_face",
    ...
    for c in ["edge", "face", "node"]:
        utm_data = np.asarray(
            [
                hgrid_nc["SCHISM_hgrid_%s_x" % c].values,
        ...
        if dl < 3:  # 1D or 2D array, just expand.
            ...
            if c3 == "MBEDP":
                ...
                for i, n in enumerate(sl):
                    vn = "%s_%s" % (v, n)
                    hnc[vn] = (
                        hnc[v].isel(MBEDP=i).squeeze().expand_dims("time").astype(float)
                    )
                    hnc[vn].attrs["mesh"] = "SCHISM_hgrid"
                    dhc = hnc[vn].dims[1].split("_")[-1]
                    if dhc == "face":
                ...
                for i in hnc[c3]:
                    ...
                    if dl == 2:  # each variable is then 1D
                        if hnc[vn].attrs["data_horizontal_center"] == "node":
                            ...
                            hnc[vn].attrs["data_horizontal_center"] == "edge"
                        ):  # on edge
                            hnc[vn].attrs["i23d"] = 7
                        else:
                            raise AttributeError(
                                "The data_horizontal_center option for %s is unavailable"
                                % v
                            )
                    else:  # I don't think that all variables are on full, but it's unclear how I can make this distinction.
                        hnc[vn].attrs["data_vertical_center"] = "half"
    ...
    mdepth.attrs["long_name"] = (
        "ocean_s_coordinate maximum depth cutoff (mixed s over z bound..."
    )
    mdepth.attrs["units"] = "meters"
    mdepth.attrs["positive"] = "down"

    cs = xr.DataArray(
        np.nan * np.ones_like(output_nc.nSCHISM_vgrid_layers),
    ...
    cs["sigma"].attrs[
        "formula_terms"
    ] = "s: sigma eta: elev depth: depth a: sigma_theta_f b: sigma..."
    ...
    output_nc.attrs["VisIT_plugin"] = (
        "https://schism.water.ca.gov/library/-/document_library/view/3476283"
    )
