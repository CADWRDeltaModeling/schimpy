...
def find_intersection(s1, s2):
    ...
    """
    det = (s1[0, 0] - s1[1, 0]) * (s2[0, 1] - s2[1, 1]) - (s1[0, 1] - s1[1, 1]) * (
        s2[0, 0] - s2[1, 0]
    )
    if det == 0.0:  # parallel
        return None
    x = (s1[0, 0] * s1[1, 1] - s1[0, 1] * s1[1, 0]) * (s2[0, 0] - s2[1, 0]) - (
        s1[0, 0] - s1[1, 0]
    ) * (s2[0, 0] * s2[1, 1] - s2[0, 1] * s2[1, 0])
    y = (s1[0, 0] * s1[1, 1] - s1[0, 1] * s1[1, 0]) * (s2[0, 1] - s2[1, 1]) - (
        s1[0, 1] - s1[1, 1]
    ) * (s2[0, 0] * s2[1, 1] - s2[0, 1] * s2[1, 0])
...
class SchismMesh(TriQuadMesh):
    ...
    def n_boundaries(self, btype=None):
        ...
        if btype is None:
            ...
            return sum(1 for b in self._boundaries if b.btype == btype)
    ...
    def n_total_boundary_nodes(self, btype):
        ...
        btype: integer
            Type of the boundary.
        """
        return sum(b.n_nodes() for b in self._boundaries if b.btype == btype)
    ...
    def _get_next_node_on_boundary_and_remove_edge(
        self, node_i, not_assigned, ccw=True
    ):
            ...
            if ccw:
                if not edge[2] == EdgeType.INTERNAL and edge[0] == node_i:
                    try:
                        ...
                        print(
                            "Attempted to remove edge {} based on node {} ({:.1f}, {:.1f}) which is not in not_assigned list".format(
                                edge_i,
                                node_i,
                                self._nodes[node_i][0],
                                self._nodes[node_i][1],
                            )
                        )
                        raise
            else:
        ...
        for i in range(len(up_path) - 1):
            ...
            if edge is None:
                ...
                for node_i in nodes_in_elem:
                    if (
                        node_i != up_path[i]
                        and node_i != up_path[i + 1]
                        and node_i not in down_path
                    ):
        ...
        for i in range(len(down_path) - 1):
            ...
            if edge is None:
                ...
                for node_i in nodes_in_elem:
                    if (
                        node_i != down_path[i]
                        and node_i != down_path[i + 1]
                        and node_i not in up_path
                    ):
    ...
    def _distance(self, nodes_i, x):
        ...
        diffs = tuple(np.subtract(nodes[i][:2], x) for i in range(len(nodes)))
    ...
    def _calculate_edge_lens(self):
        ...
        self._edge_lengths = np.linalg.norm(
            self._nodes[edges[:, 0], :2] - self._nodes[edges[:, 1], :2], axis=1
        )
    ...
    def _calculate_centroids(self):
        ...
        if self._polygons is None:
            self._create_2d_polygons()
        self._centroids = np.array([p.centroid.coords[0] for p in self._polygons])
    ...
    def plot_edges(self, var, ax=None, size=500, inpoly=None, **kwargs):
        ...
        if inpoly is not None:
            ...
            sca_plt = ax.scatter(xy[:, 0], xy[:, 1], c=var[inpoly], s=size, **kwargs)
        else:
            xy = xy[:, :2]
    ...
    def read_elems(self, f):
        for elem_i in range(self._mesh.n_elems()):
            ...
            if type_elem < 3 or type_elem > 4:
                self._logger.error("Error reading: %s", line)
                self._logger.error("Only triangular or quadrilateral are supported.")
                raise ValueError("Element block is corrupt")
        ...
        try:
            ...
            for i in range(n_land_boundaries):
                ...
                for j in range(n_nodes):
                    ...
                    node = int(tkns[0]) - 1  # Zero based
                    nodes.append(node)
                self._mesh.add_boundary(nodes, BoundaryType.LAND)
        except ValueError:
            raise ValueError("Boundary block is corrupt")
            ...
            for line in file_object:
                ...
                if row[0] == "ND":
                    ...
                    try:
                        ...
                        print(
                            "Bad node: . This can be caused by deleting nodes in SMS and failing to renumber:".format(
                                node_i
                            )
                        )
                        raise
                ...
                if row[0] == "NS":
                    ...
                    if nodes[0] < 0:
                        ...
                        if nodestring_option == "open":
                            self.mesh.add_boundary(bound_nodes, BoundaryType.OPEN)
                        elif nodestring_option == "land":
                            self.mesh.add_boundary(bound_nodes, BoundaryType.LAND)
                        else:
...
class SchismMeshGr3Writer(SchismMeshWriter):
    ...
    def _write_boundary(self):
        ...
        f.write(buf)
        buf = (
            "%d = Total number of open boundary nodes\n"
            % mesh.n_total_boundary_nodes(BoundaryType.OPEN)
        )
        f.write(buf)
        openbound_count = 0
        ...
        landbound_count = 0
        for bndry in mesh._boundaries:
            if bndry.btype == BoundaryType.LAND or bndry.btype == BoundaryType.ISLAND:
                landbound_count += 1
...
class SchismMeshNetcdfWriter(SchismMeshWriter):
    ...
    def write(self, *args, **kwargs):
        ...
        ds_node_x = xr.DataArray(
            nodes[:, 0], dims=["n_hgrid_node"], name="hgrid_node_x"  # coords=[n_node],
        )
        ...
        ds_node_y = xr.DataArray(
            nodes[:, 1], dims=["n_hgrid_node"], name="hgrid_node_y"  # coords=[n_node],
        )
        ...
        ds_values = xr.DataArray(
            depth, dims=["n_hgrid_node"], name="depth"  # coords=[n_node],
        )
        ...
        ds_elem_contour_x = xr.DataArray(
            contour[:, :, 0],  # coords=[n_face,n_face_node],
            dims=["n_hgrid_face", "n_face_node"],
            name="hgrid_contour_x",
        )
        ...
        ds_elem_contour_y = xr.DataArray(
            contour[:, :, 1],  # coords=[n_face,n_face_node],
            dims=["n_hgrid_face", "n_face_node"],
            name="hgrid_contour_y",
        )
        ...
        schism_face_node = (
            np.asarray(
                [
                    (
                        np.append(e.astype("int32"), np.nan)
                        if len(e) < 4
                        else e.astype("int32")
                    )
                    for e in mesh.elems
        ...
        ds_face_nodes = xr.DataArray(
            schism_face_node,  # coords=[n_face, n_face_node],
            dims=["n_hgrid_face", "n_face_node"],
            name="hgrid_face_nodes",
        ).astype("int32")
        ...
        ds_edge_nodes = xr.DataArray(
            schism_edge_nodes,  # coords=[n_edge,np.arange(2)],
            dims=["n_hgrid_edge", "two"],
            name="hgrid_edge_nodes",
        ).astype("int32")
        ds_edge_nodes.attrs["long_name"] = (
            "Map every edge to the two nodes that it connects"
        )
        ...
        ds_face_x = xr.DataArray(
            face_x, dims=["n_hgrid_face"], name="hgrid_face_x"  # coords=[n_face],
        )
        ...
        ds_face_y = xr.DataArray(
            face_y, dims=["n_hgrid_face"], name="hgrid_face_y"  # coords=[n_face],
        )
        ...
        ds_edge_x = xr.DataArray(
            edge_x, dims=["n_hgrid_edge"], name="hgrid_edge_x"  # coords=[n_edge],
        )
        ...
        ds_edge_y = xr.DataArray(
            edge_y, dims=["n_hgrid_edge"], name="hgrid_edge_y"  # coords=[n_edge],
        )
        ...
        if hasattr(mesh, "vmesh"):
            ...
            ds_node_bottom = xr.DataArray(
                node_bottom_index,  # coords=[n_node],
                dims=["n_hgrid_node"],
                name="node_bottom_index",
            )
            ...
            ds_ele_bottom = xr.DataArray(
                ele_bottom_index,  # coords=[n_face],
                dims=["n_hgrid_face"],
                name="ele_bottom_index",
            )
            ...
            ds_edge_bottom = xr.DataArray(
                edge_bottom_index,  # coords=[n_edge],
                dims=["n_hgrid_edge"],
                name="edge_bottom_index",
            )
            ...
            z = mesh.z
            ds_z = xr.DataArray(
                z,  # coords=[n_node,n_vert_levels],
                dims=["n_hgrid_node", "n_vgrid_layers"],
                name="z",
            )
...
class SchismMeshIoFactory(object):
    ...
    registered_readers = {"gr3": "SchismMeshGr3Reader", "sms": "SchismMeshSmsReader"}
    registered_writers = {
        "gr3": "SchismMeshGr3Writer",
...
def write_mesh(
    mesh, fpath_mesh, node_attr=None, write_boundary=False, crs=None, **kwargs
):
    ...
    if fpath_mesh.endswith(("gr3", "ll", "ic")):
        ...
        writer.write(
            mesh=mesh, fpath=fpath_mesh, node_attr=node_attr, crs=crs, **kwargs
        )
    elif fpath_mesh.endswith(("nc")):
        writer = SchismMeshIoFactory().get_writer("nc")
        writer.write(mesh=mesh, fpath=fpath_mesh, node_attr=node_attr, **kwargs)
    else:
        raise ValueError("Unsupported extension")
...
def compare_mesh(mesh1, mesh2, dist_threshold=None):
    ...
    indices : ndarray of int
        Indices of the nearest nodes in mesh1
    dist : ndarray of float
        Distances between the nearest nodes
    """

    # Build KDTree on mesh1 nodes (x, y only)
    mesh1_idx = cKDTree(mesh1.nodes[:, :2])

    # Bulk query for all mesh2 nodes
    dist, indices = mesh1_idx.query(mesh2.nodes[:, :2])

    if dist_threshold is not None:
        overlap_fraction = np.sum(dist <= dist_threshold) / len(dist) * 100
        print(
            "The overlapping nodes account for %.2f percent of total nodes"
            % overlap_fraction
        )
