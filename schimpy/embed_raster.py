...
def embed_raster(
    ...
    **kwargs,
    ...
    fine_good = np.where(np.isnan(dem_fine), 0.0, 1.0)
    ...
    dem_final = contour_smooth2d(
        dem_mixed, scales, max_time_final, nstep, report_interval
    )
    ...
    if plot:
        ...
        ax0.imshow(
            dem_final, vmin=vmin, vmax=vmax, origin="upper", extent=ds_coarse.extent
        )
        ...
        ax1.imshow(
            dem_coarse, vmin=vmin, vmax=vmax, origin="upper", extent=ds_coarse.extent
        )
        cs1 = ax1.contour(
            dem_coarse,
            levels,
        ...
        ax2.imshow(
            dem_fine, vmin=vmin, vmax=vmax, origin="upper", extent=ds_fine.extent
        )
        cs2 = ax2.contour(
            dem_fine,
            levels,
